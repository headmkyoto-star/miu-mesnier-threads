...
GITHUB_RAW_BASE = "https://raw.githubusercontent.com/headmkyoto-star/miu-mesnier-threads/main/"
GITHUB_API_BASE = "https://api.github.com/repos/headmkyoto-star/miu-mesnier-threads/contents/"
...
def generate_post():
    ...
    prompt = f"""京都河原町のリラクゼーションサロン「みうメスニエル」の若い女性セラピストとして、Threadsの短い営業投稿を作成してください。
...
def is_rest_day_jp():
    """投稿は月・火・金・土のみ。水・木・日ならTrue（JST基準）"""
    import datetime
    today = (datetime.datetime.utcnow() + datetime.timedelta(hours=9)).date()
    if today.weekday() in (2, 3, 6):
        print(f"😴 {today} は水・木・日なので投稿お休み")
        return True
    return False
